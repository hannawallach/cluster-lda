...
public class ClusterPermuteTestEvaluator {
  ...
  public static void getClusteredCorpus(Corpus docs, String fileName) {
	...
	public static void main(String[] args) {
		...
		if (args.length != 9) {
			System.out.println("Usage: ClusterPermuteTestEvaluator <train_data> <test_data> <train_clusters> <alpha_file> <theta_file> <num_clusters> <use_doc_counts> <prior_type> <output_dir>");
			System.exit(1);
		}
    ...
    String permutedStateFileName = outputDir + "/permuted_test_state.txt";

		// permute the test data and compute P(test data under the UP)

		String logProbFileName = outputDir + "/log_prob_test_permutations.txt";
		...
		try {
			...
			for (int p=0; p<20; p++) {
				...
				// now have a permuted test corpus
				...
				testDocs.printAssignments(z, -1, 0, permutedStateFileName + "." + p);

				ct.initializeEval(permutedStateFileName + "." + p, testDocs);
